...
{!! sprintf(Form::label('category_id', '%s', ['class' => 'col-sm-4 control-label']), trans('phrases.category') . ' <span class="text-danger">*</span>') !!}
	<div class="col-md-6">
		@if (isset($transaction))
			{!! Form::select('category_id', ['' => trans('phrases.select_category')] + Modules\Accounting\Entities\Category::where('type', $transaction->type)->get()->lists('category_name', 'id'), $transaction->category_id , ['class' => 'form-control', 'id' => "category_input"]) !!}
		@else
